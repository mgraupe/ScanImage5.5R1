classdef ConfigurationEditor < most.Gui 
    ...
    methods
        ...
        function constructAddlCompsPnl(obj)
            if ~most.idioms.isValidObj(obj.hAdditionalComponentsPanel)
                ...
                b1 = uicontrol(args{:},'string', 'Configure Photostimulation','position',[46 ph-256 300 50],'callback',@(varargin)addComp('wizardAddedPhotostim','Photostim'));
                b2 = uicontrol(args{:},'string', 'Configure Closed Loop Experiment Outputs','position',[46 ph-316 300 50],'callback',@(varargin)addComp('wizardAddedIntegration','IntegrationRoiOutputs'));
                uicontrol(args{:},'string', 'Finish and Run ScanImage','position',[46 ph-376 300 50],'callback',@obj.pbApplyCb);
    ...
    methods
        ...
        function deleteAllPages(obj)
            delete([obj.hResScanPages{:}]);
            delete([obj.hLinScanPages{:}]);
            delete([obj.hSlmScanPages{:}]);
        ...
        function initDaqInfo(obj)
            ...
            for i = 1:numel(obj.availableRios)
                ...
                if ~isempty(strfind(pn,'NI517'))
                    ...
                    obj.rioInfo.(nm).adapterModule = strrep(obj.rioInfo.(nm).adapterModule,'(-01)DC','');
        ...
        function refreshPages(obj,varargin)
            ...
            if isempty(remHdgs)
                ...
                [obj.hSIPage, siIdx] = isPageThere(...
                    'ScanImage', obj.hSIPage, @scanimage.guis.configuration.SIPage);
                [obj.hShuttersPage, shuttersIdx] = isPageThere(...
                    'Shutters', obj.hShuttersPage, @scanimage.guis.configuration.ShuttersPage);
                [obj.hBeamsPage, beamsIdx] = isPageThere(...
                    'Beams', obj.hBeamsPage, @scanimage.guis.configuration.BeamsPage);
                removeIdx = removeIdx | siIdx | shuttersIdx | beamsIdx;
            end
            
            % scanner pages
            if most.idioms.isValidObj(obj.hSIPage)
                expectedNames = obj.scannerNames;
            else
                expectedNames = {};
            end
            
            [obj.hResScanPages, rsHdgs] = isNamedPageThere(...
                'ResScan', obj.hResScanPages,...
                @scanimage.guis.configuration.ResScanPage, expectedNames);
            [obj.hLinScanPages, linHdgs] = isNamedPageThere(...
                'LinScan', obj.hLinScanPages,...
                @scanimage.guis.configuration.LinScanPage, expectedNames);
            [obj.hSlmScanPages, slmHdgs] = isNamedPageThere(...
                'SlmScan', obj.hSlmScanPages,...
                @scanimage.guis.configuration.SlmScanPage, expectedNames);
            removeIdx = removeIdx | rsHdgs | linHdgs | slmHdgs; % all non scanner pages 
            %repopulate scannerMap
            obj.scannerMap.remove(obj.scannerMap.keys);
            for i=1:length(obj.hResScanPages)
                page = obj.hResScanPages{i};
                obj.scannerMap(page.scannerName) = page;
            end
            
            for i=1:length(obj.hLinScanPages)
                page = obj.hLinScanPages{i};
                obj.scannerMap(page.scannerName) = page;
            end
            
            for i=1:length(obj.hSlmScanPages)
                page = obj.hSlmScanPages{i};
                obj.scannerMap(page.scannerName) = page;
            end
            
            [obj.hMotorsPage, motorsIdx] = isPageThere('Motors', obj.hMotorsPage,...
                @scanimage.guis.configuration.MotorsPage);
            [obj.hFastZPage, fastzIdx] = isPageThere('FastZ', obj.hFastZPage,...
                @scanimage.guis.configuration.FastZPage);
            [obj.hPhotostimPage, stimIdx] = isPageThere('Photostim', obj.hPhotostimPage,...
                @scanimage.guis.configuration.PhotostimPage);
            [obj.hIntegrationRoiOutputsPage, integrationIdx] = isPageThere(...
                'IntegrationRoiOutputs',obj.hIntegrationRoiOutputsPage, ...
                @scanimage.guis.configuration.IntegrationROIOutputsPage);
            [obj.hThorLabsBScope2Page, bscope2Idx] = isPageThere('Thorlabs BScope2', ...
                obj.hThorLabsBScope2Page, ...
                @scanimage.guis.configuration.ThorLabsBScope2Page);
            [obj.hThorLabsECUScannersPage, ecuIdx] = isPageThere('Thorlabs ECU1', ...
                obj.hThorLabsECUScannersPage, ...
                @scanimage.guis.configuration.ThorLabsECUScannersPage);
            [obj.hPMTControllersPage, pmtIdx] = isPageThere('GenericPmtController', ...
                obj.hPMTControllersPage, ...
                @scanimage.guis.configuration.PMTControllersPage);
            [obj.hLSCPureAnalogPage, lscIdx] = isPageThere('LSC Pure Analog', ...
                obj.hLSCPureAnalogPage, ...
                @scanimage.guis.configuration.LSCPureAnalogPage);
            
            removeIdx = removeIdx | motorsIdx | fastzIdx | stimIdx |...
                integrationIdx | bscope2Idx | ecuIdx | pmtIdx | lscIdx;
            ...
            for i = 1:numel(remHdgs) 
                obj.hOtherPages{end+1} = ...
                    scanimage.guis.configuration.GenericPage(obj,remHdgs{i});
            end
            % Need to have it here to appear as all pages and actually
            % create, worked before becuase it was being put into other
            % pages...
            obj.hAllPages = [obj.hSIPage, obj.hShuttersPage, obj.hBeamsPage,...
                obj.hResScanPages{:}, obj.hLinScanPages{:}, obj.hSlmScanPages{:},...
                obj.hMotorsPage, obj.hFastZPage, obj.hPhotostimPage,...
                obj.hIntegrationRoiOutputsPage, obj.hThorLabsBScope2Page,... 
                obj.hThorLabsECUScannersPage, obj.hPMTControllersPage,...
                obj.hLSCPureAnalogPage, obj.hOtherPages{:}];
            if obj.isWizardMode
                ...
                btns = [{'General ScanImage Settings' 'Shutter Configuration',...
                    'Power Modulation (Beams)'} spn extraComps {'Additional Components'}];
            ...
            switch obj.hActivePage
                case obj.hSIPage
                    ...
                    if ~all(vs)
                        ...
                        res_idx = ~vs & strcmpi(obj.scannerTypes, 'resonant');
                        
                        res_num = sum(res_idx);
                        res_pages = cell(1,res_num);
                        res_names = obj.scannerNames(res_idx);
                        for i=1:res_num
                            res_pages{i} = ...
                                scanimage.guis.configuration.ResScanPage(obj,...
                                res_names{i}, true);
                        end
                        obj.hResScanPages = [obj.hResScanPages res_pages];
                        
                        lin_idx = ~vs & strcmpi(obj.scannerTypes, 'linear');
                        lin_num = sum(lin_idx);
                        lin_pages = cell(1,lin_num);
                        lin_names = obj.scannerNames(lin_idx);
                        for i=1:lin_num
                            lin_pages{i} = ...
                                scanimage.guis.configuration.LinScanPage(obj,...
                                lin_names{i}, true);
                        end
                        obj.hLinScanPages = [obj.hLinScanPages lin_pages];
                        
                        slm_scanners = ~vs & strcmpi(obj.scannerTypes, 'slm');
                        slm_num = sum(slm_scanners);
                        slm_pages = cell(1,slm_num);
                        slm_names = obj.scannerNames(slm_scanners);
                        for i=1:slm_num
                            slm_pages{i} = ...
                                scanimage.guis.configuration.SlmScanPage(obj, ...
                                slm_names{i}, true);
                        end
                        obj.hSlmScanPages = [obj.hSlmScanPages slm_pages];
                    end
